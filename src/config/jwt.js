...
export const generateToken = (payload, expDate = "2d") => {
    return new Promise((resolve, reject) => {
        jwt.sign(payload, process.env.VALIDATION_KEY, { expiresIn: expDate }, (err, token) => {
            if (err)
                reject(err);
            else
                resolve(token);
...
export const verifyToken = (token) => {
    return new Promise((resolve, reject) => {
        jwt.verify(token, process.env.VALIDATION_KEY, (err, payload) => {
            if (err)
                reject(err);
            else
                resolve(payload);
