...
import { pgTable, uuid, text, date, pgEnum, boolean, timestamp, uniqueIndex, doublePrecision, primaryKey, check, index, integer, bigint, } from "drizzle-orm/pg-core";
//declaring an enum
...
*/
//tagsUsers
export const tagsUsers = pgTable("tags_users", {
...
export const tags = pgTable("tags", {
    ...
    lowercaseCheck: check("lowercase_check", sql `${table.tagName} = lower(${table.tagName})`),
}));
//joint table of tag templates and tag categories
export const tagsTagCats = pgTable("tags_tag_cats", {
    tagName: text("tag_name")
        .notNull()
        .references(() => tags.tagName),
...
export const blocks = pgTable("blocks", {
    ...
    return {
        ...
        checkUniqueBlocks: uniqueIndex("unique_blocks").on(table.blockingUserId, table.blockedUserId),
...
export const chats = pgTable("chats", {
    ...
    lastMessageSender: uuid("last_message_sender").references(() => users.userId),
    ...
    return {
        checkUniqueParticipants: uniqueIndex("unique_participants").on(table.participant1, table.participant2),
...
export const chatMessages = pgTable("chat_messages", {
    ...
    return {
         ...
         */
        chatMessageIndex: index("chat_message_index").on(table.chatId, table.sentDate, table.receivedDate),
        //this index is used for fetching undelivered messages
        chatMessageIndex2: index("chat_message_index2").on(table.recipientId, table.eventId),
        eventIdFormat: check("event_id_format", sql `${table.eventId} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}:[0-9]{1}:\d{13}\.\d{3}$'`),
    };
});
...
export const connectionRequestRelations = relations(connectionRequests, ({ one }) => ({
    sender: one(users, {
