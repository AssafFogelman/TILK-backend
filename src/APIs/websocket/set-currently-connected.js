...
export async function setCurrentlyConnected({ token }, callback) {
    try {
        ...
        const lastReceivedEventId = socket.handshake.auth.lastReceivedEventId; //last successfully received event id
        if (lastReceivedEventId) {
            // this is a reconnection. the user is in the app and needs to see the new events now!
            for (const event of await fetchMissedEventsFromDatabase(userId, lastReceivedEventId)) {
        ...
        console.log("error trying to set currentlyConnected or deliver pending messages from the database: ", error);
        callback({
            error: new Error("error trying to set currentlyConnected or deliver pending messages from the database: ", { cause: error }),
        });
    }
}
