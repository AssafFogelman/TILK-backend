...
import { chatMessages, chats, unreadEvents, users, } from "../../drizzle/schema.js";
import { db } from "../../drizzle/db.js";
import { TilkEventType } from "../../backend-types/TilkEventType.js";
export async function onMessagesRead({ chatId, lastUnreadMessageReceivedDate }, callback) {
    try {
        ...
        if (!chatId)
            throw new Error("no chatId"); //if there is no chatId, report an error
        ...
        const lastReadMessage = (await db
            .update(chatMessages)
            .set({ unread: false })
            .where(and(eq(chatMessages.chatId, chatId), ne(chatMessages.senderId, userId), lte(chatMessages.receivedDate, new Date(lastUnreadMessageReceivedDate)), eq(chatMessages.unread, true) //target only the received message that was unread
        ))
            .returning({
        ...
        const isParticipant1 = (await db.query.chats.findFirst({
        ...
        await db
            ...
            ...(lastReadMessage && {
                [isParticipant1 ? "lastReadMessageP1" : "lastReadMessageP2"]: lastReadMessage.messageId,
            }),
        })
            .where(eq(chats.chatId, chatId));
        ...
        if (!lastReadMessage)
            return; //no message was read. no need to update.
        //delete the unreadEvents from the DB
        await db
            .delete(unreadEvents)
            .where(and(eq(unreadEvents.eventType, TilkEventType.MESSAGE), eq(unreadEvents.userId, lastReadMessage.recipientId), eq(unreadEvents.chatId, chatId)));
