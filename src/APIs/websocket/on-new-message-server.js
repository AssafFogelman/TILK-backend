...
import { chatMessages, 
/*undeliveredEvents,*/ users, } from "../../drizzle/schema.js";
...
export async function onNewMessage(message, callback) {
    try {
        if (!message) {
        ...
        const { eventId, ...savedMessage } = (await db
            .insert(chatMessages)
            .values({
            chatId,
            sentDate,
            ...
            recipientId,
            gotToServer,
            eventId: `${recipientId}:${gotToServer + ""}.${getNextSequence(gotToServer)
                .toString()
        ...
        if (recipientUser?.expoPushToken) {
            ...
            if (!Expo.isExpoPushToken(recipientUser.expoPushToken)) {
                throw new Error(`Invalid Expo push token ${recipientUser.expoPushToken}`);
            }
...
function getNextSequence(timestamp) {
    ...
    if (messageSequences.size > 1000) {
        const oldKeys = Array.from(messageSequences.keys()).filter((key) => parseInt(key) < timestamp - 1000);
