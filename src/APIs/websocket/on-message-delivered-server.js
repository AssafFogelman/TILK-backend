...
export async function messageDelivered({ receivedDate, messageId, chatId }, callback) {
    try {
        const socket = this;
        const userId = socket.data.userId;
        // Mark chat message as delivered
        const deliveredMessage = (await db
            .update(chatMessages)
        ...
        if (sender?.currentlyConnected) {
            console.log("sender is online, emitting that the sent message was delivered");
            console.log(`Attempting to emit confirmation to sender ${deliveredMessage.senderId}`);
            console.log("");
            ...
            io.to(deliveredMessage.senderId).emit("messageDelivered", deliveredMessage);
        }
