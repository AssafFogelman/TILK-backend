 ...
 */
export const createToken = async (c) => {
    try {
        const { phoneNumber, code, hash, } = await c.req.json();
        //is the hash valid?
        const hashValid = await compareHash(phoneNumber + code + process.env.VALIDATION_KEY, hash);
        if (!hashValid)
