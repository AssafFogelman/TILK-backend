...
import { connections, users, connectionRequests, blocks, tagsUsers, tags, chats, } from "../drizzle/schema.js";
...
*/
export const getConnectionsList = async (c) => {
    try {
        ...
        const receivedRequests = (await db
            ...
            .leftJoin(blocks, or(and(eq(blocks.blockingUserId, userId), eq(blocks.blockedUserId, users.userId)), and(eq(blocks.blockingUserId, users.userId), eq(blocks.blockedUserId, userId))))
            .where(and(eq(connectionRequests.recipientId, userId), 
        ...
        not(isNull(users.smallAvatar)), not(isNull(users.nickname)), not(isNull(users.gender)), not(isNull(users.biography))))
            .orderBy(desc(connectionRequests.requestDate)));
        // Get connected users
        const connectedUsers = (await db
            ...
            .from(connections)
            .innerJoin(users, or(eq(connections.connectedUser1, users.userId), eq(connections.connectedUser2, users.userId)))
            .leftJoin(blocks, or(and(eq(blocks.blockingUserId, userId), eq(blocks.blockedUserId, users.userId)), and(eq(blocks.blockingUserId, users.userId), eq(blocks.blockedUserId, userId))))
            .where(and(
        // user is connected to the current user
        or(eq(connections.connectedUser1, userId), eq(connections.connectedUser2, userId)), 
        ...
        // so if you have performance issues, you can remove the not(isNull())s
        not(isNull(users.smallAvatar)), not(isNull(users.nickname)), not(isNull(users.gender)), not(isNull(users.biography))))
            .orderBy(desc(connections.connectionDate)));
        ...
        const sentRequests = (await db
            ...
            .leftJoin(blocks, or(and(eq(blocks.blockingUserId, userId), eq(blocks.blockedUserId, users.userId)), and(eq(blocks.blockingUserId, users.userId), eq(blocks.blockedUserId, userId))))
            .where(and(eq(connectionRequests.senderId, userId), 
        //user isn't blocked or blocking
        isNull(blocks.blockId), 
        ...
        const userTags = userIds.length > 0
            ? await db
      ...
      END`.as("unread"),
        ...
        const lastMessagesMap = new Map(lastMessages.map((msg) => [
        ...
        return c.json({ message: 'error in "get-connections-list" route:' + error }, 401);
    }
};
