...
returns the hash
*/
export const sendSms = async (c) => {
    try {
        const { phoneNumber, uniqueOSCode, } = await c.req.json();
        ...
        if (process.env.NODE_ENV?.toLowerCase() !== "production") {
            if (uniqueOSCode === "82a517fa0d9bfe4b" ||
        ...
        todaysRequests = todaysRequests.filter((request) => request.weekDay >= new Date().getDay() - 1 &&
        ...
        const hash = await createHash(phoneNumber + code + process.env.VALIDATION_KEY);
        //send SMS
...
function areTooManyInstances(arr, uniqueOSCode, restrictNumber) {
    let count = 0;
    // Count how many instances of the uniqueOSCode there are
    arr.forEach((sendSMSRequest) => {
        if (sendSMSRequest.uniqueOSCode === uniqueOSCode)
            count++;
    });
