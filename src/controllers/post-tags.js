...
import { tags, tagsUsers, } from "../drizzle/schema.js";
import { eq, inArray } from "drizzle-orm";
export const postTags = async (c) => {
    try {
        const { userId } = c.get("tokenPayload");
        const tagsArray = (await c.req.json());
        //do the tags exist in the database?
        // Check if all tags in tagsArray exist in the tags table
        const existingTags = new Set((await db
            .select({ tagName: tags.tagName })
            .from(tags)
            .where(inArray(tags.tagName, tagsArray))).map((tag) => tag.tagName));
        ...
        if (tagsArray.length > 0) {
            await db.insert(tagsUsers).values(tagsArray.map((tagName) => ({
                userId: userId,
