...
//get the chats list
export async function getChatsList(c) {
    try {
        const { userId } = c.get("tokenPayload");
        const chatsList = await db.query.chats.findMany({
            where: and(or(eq(chats.participant1, userId), eq(chats.participant2, userId)), not(exists(db
                .select()
                .from(blocks)
                .where(and(eq(blocks.blockingUserId, userId), or(eq(blocks.blockedUserId, chats.participant1), eq(blocks.blockedUserId, chats.participant2))))))),
            with: {
                participant1: {
                    columns: {
                        userId: true,
        ...
        const filteredChatsList = chatsList.filter((chat) => chat.participant1.nickname &&
        ...
        const formattedChats = filteredChatsList.map((chat) => {
            const otherUser = chat.participant1.userId === userId
                ? {
