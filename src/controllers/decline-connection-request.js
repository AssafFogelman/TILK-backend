...
export const declineConnectionRequest = async (c) => {
    try {
        ...
        await db
            ...
            .where(and(eq(connectionRequests.recipientId, userId), eq(connectionRequests.senderId, senderId)));
        return c.json({ message: "connection request declined" }, 200);
    }
    catch (error) {
        console.log("error in decline-connection-request route:", error);
        return c.json({ message: "error in decline-connection-request route:" + error }, 500);
    }
};
