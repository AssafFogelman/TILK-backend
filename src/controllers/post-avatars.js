...
export const postAvatars = async (c) => {
    try {
        const { userId } = c.get("tokenPayload");
        const { avatars } = (await c.req.json());
        ...
        console.log("Received data:", avatars.map((avatar) => JSON.stringify(avatar).substring(0, 100) + "...")); // Log a preview of the received data
        ...
        const originalAvatarTempPaths = (await Promise.all(avatars.map(async (avatar, index) => {
            if (!avatar)
                return null;
