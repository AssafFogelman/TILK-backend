 ...
 */
export const getKnn = async (c) => {
    try {
        const { userId } = c.get("tokenPayload");
        let { latitude, longitude, limit = 20 } = (await c.req.json());
    ...
    `;
        ...
        return c.json({ message: "error retrieving nearest neighbor user data: ", error }, 401);
    }
};
