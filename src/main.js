...
import { tagCategories, tags, tagsTagCats, tagsUsers, users, } from "./drizzle/schema.js";
const exampleUsers = [
    {
...
async function main() {
    try {
        ...
        const tagsWithCategoriesAndUsers = await db
            ...
            categories: sql `array_agg(distinct ${tagCategories.categoryName})`,
            users: sql `array_agg(distinct ${users.phoneNumber})`,
        })
            .from(tags)
            .leftJoin(tagsTagCats, eq(tags.tagName, tagsTagCats.tagName))
            .leftJoin(tagCategories, eq(tagsTagCats.tagCategoryId, tagCategories.tagCategoryId))
